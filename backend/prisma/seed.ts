import { PrismaClient, Gender, UserRole, SettingDataType, AuditAction, AuditEntity, AuditCategory, LogSeverity, NotificationType, NotificationCategory, ContactMethod, ConsentType, ConsentMethod } from '@prisma/client';
import { patientSeedData, referringGPSeedData } from './seedData/patients';
import { studySeedData, additionalStudySeedData } from './seedData/studies';
import { reportSeedData } from './seedData/reports';
import { appointmentSeedData, referralSeedData } from './seedData/appointments';
...
async function main() {
  ...
  try {
    ...
    for (const gp of referringGPSeedData) {
    ...
    for (const referralData of referralSeedData) {
    ...
    for (const studyData of studySeedData) {
      const patient = await prisma.patient.findFirst({
    ...
    for (const additionalStudy of additionalStudySeedData) {
      const patient = await prisma.patient.findFirst({
    ...
    for (const appointment of upcomingAppointments.slice(0, 5)) {
      await prisma.notification.create({
        data: {
          ...
          sentVia: [ContactMethod.SMS, ContactMethod.EMAIL],
    ...
    for (const user of patientUsers.slice(0, 8)) {
      ...
      await prisma.consent.create({
        data: {
          ...
          isConsented: true,
          consentMethod: ConsentMethod.ONLINE,
      ...
      await prisma.consent.create({
        data: {
          userId: user.id,
          consentType: ConsentType.COMMUNICATION,
          consentVersion: 'v1.0',
