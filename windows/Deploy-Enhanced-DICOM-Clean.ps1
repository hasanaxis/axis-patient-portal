# Deploy Enhanced DICOM Receiver to Windows Server - CLEAN VERSION
...
Write-Host ""
Write-Host "1. Stopping current DICOM service..." -ForegroundColor Yellow
try {
...
Write-Host ""
Write-Host "2. Backing up current DICOM receiver..." -ForegroundColor Yellow
...
Write-Host ""
Write-Host "3. Creating enhanced DICOM receiver..." -ForegroundColor Yellow
...
Write-Host ""
Write-Host "4. Updating environment configuration..." -ForegroundColor Yellow
...
"@
    ...
    Write-Host "WARNING: Supabase key not provided - cloud integration will be disabled" -ForegroundColor Yellow
    Write-Host "   Add SUPABASE_SERVICE_KEY to .env file to enable cloud sync" -ForegroundColor Gray
}

# 5. Test the enhanced receiver
Write-Host ""
Write-Host "5. Testing enhanced DICOM receiver..." -ForegroundColor Yellow
try {
    ...
    if ($testJob.State -eq "Running") {
        Write-Host "SUCCESS: Enhanced DICOM receiver test passed" -ForegroundColor Green
        Stop-Job $testJob
        Remove-Job $testJob
    } else {
        Write-Host "ERROR: Enhanced DICOM receiver test failed" -ForegroundColor Red
        Receive-Job $testJob
        Remove-Job $testJob
        return
    }
...
Write-Host ""
Write-Host "6. Updating service configuration..." -ForegroundColor Yellow
...
Write-Host ""
Write-Host "7. Starting enhanced DICOM service..." -ForegroundColor Yellow
try {
    ...
    if ($serviceStatus.Status -eq "Running") {
        Write-Host "SUCCESS: Enhanced DICOM service started" -ForegroundColor Green
    } else {
...
Write-Host ""
Write-Host "8. Service status verification..." -ForegroundColor Yellow
$services = Get-Service AxisHL7, AxisDICOM
$services | Format-Table Name, Status, StartType

# 9. Test connectivity
Write-Host ""
Write-Host "9. Testing port connectivity..." -ForegroundColor Yellow
...
$hl7Status = if($hl7Port){'OPEN'}else{'CLOSED'}
$dicomStatus = if($dicomPort){'OPEN'}else{'CLOSED'}

Write-Host "   HL7 Port 2575: $hl7Status" -ForegroundColor $(if($hl7Port){'Green'}else{'Red'})
Write-Host "   DICOM Port 104: $dicomStatus" -ForegroundColor $(if($dicomPort){'Green'}else{'Red'})
...
$dicomEndpoint = "$ServerIP" + ":104"
Write-Host "   Configure modalities to send DICOM to $dicomEndpoint" -ForegroundColor White
